...
class Main extends PluginBase implements Listener {
    ...
    public function getUpgradeForm(Player $player) : MenuForm{
    	...
    	if($level === null && $type === null) {
    		...
    		$message = "The ore spawner you are holding is the level $level, do you want to upgrade to level " . $level + 1 . " with \$$cost?\nYour money: " . $this->eco->myMoney($player) . "\n§cWaring: You need to leave 1 space in your inventory!§r";
    		$options = [
				...
				switch($data) {
					case 0:
						if($this->eco->myMoney($player) >= $cost) {
							...
							$player->sendMessage("§aTốt, §bOreSpawner §acủa bạn đã được nâng cấp lên cấp " . $level + 1);
						} else {
    ...
    public function getMainForm(Player $player) : MenuForm{
    	...
    	if($this->getServer()->isOp($player->getName())) {
    		$options = array_merge($options, [new MenuOption("Give OreSpawner (Op show only)")]);
   	 }
    	return new MenuForm(
			"OreSpawner Menu",
			"",
